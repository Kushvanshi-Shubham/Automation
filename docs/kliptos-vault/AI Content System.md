...
### 3. Teach a style (in progress)
Upload 2–20 of your reels (existing Footage flow — already Whisper-transcribed) → `style.learn` task mines pace/hooks/transcripts → one LLM pass produces a recipe + caption/music defaults → appears in Create as a personal format (`user:<uuid>`). Table `user_formats` (migration 0011), `/styles` router, max 5 per user. Upload-only by design: Kliptos never scrapes reels.

### 4. AI presenter (HeyGen, scaffold)
BYO HeyGen key through the existing encrypted `user_api_keys` flow + avatar/voice IDs; `services/heygen.py` generate→poll→download client. Blocked on owner acquiring a HeyGen account for live testing; render-lane integration and credit pricing (above stock renders — see [[Pricing]]) follow.
