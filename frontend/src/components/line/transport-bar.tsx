 ...
 * counts. Monospace appears only on real numbers; keys 1–4 travel.
 */
...
import { StSignal, StDesk, StRail, StAir, StVault } from "@/components/line/stencils"

interface VideoT { id: string; status: string; youtube_video_id: string | null; published_at: string | null }

export const STATIONS = [
  { key: "discover", label: "Discover", href: "/dashboard/topics", Icon: StSignal },
  { key: "create", label: "Create", href: "/dashboard/studio", Icon: StDesk },
  { key: "production", label: "Production", href: "/dashboard/rail", Icon: StRail },
  { key: "library", label: "Library", href: "/dashboard", Icon: StAir },
] as const
...
export default function TransportBar() {
  ...
  return (
    <nav
      aria-label="Kliptos"
      style={{
        position: "fixed", left: 0, right: 0, bottom: 0, zIndex: 60, height: 48,
    ...
    >
      ...
      <div style={{ display: "flex", alignItems: "center", padding: "0 14px", borderRight: `1px solid ${L.rule}` }}>
        ...
        <img src="/brand/kliptos-logo-2k.jpeg" alt="Kliptos" style={{ width: 22, height: 22, borderRadius: 3, objectFit: "cover" }} />
      </div>
        ...
        {STATIONS.map((s, i) => {
          ...
          return (
            ...
            >
              ...
              <s.Icon size={15} color={isActive ? L.ink : L.ash} />
              <span style={{ fontSize: 13, fontWeight: isActive ? 600 : 400, color: isActive ? L.ink : L.ash }}>{s.label}</span>
              {l && (
                <span style={{ fontFamily: mono, fontSize: 10, color: l.color, border: `1px solid ${l.color}55`, padding: "1px 5px", borderRadius: 2 }}>
                  {l.label}
                </span>
              )}
            </Link>
      ...
      <div style={{ display: "flex", alignItems: "center", borderLeft: `1px solid ${L.rule}` }}>
        <Link href="/dashboard/clips"
          ...
          {isFootage && <span aria-hidden style={{ position: "absolute", top: -1, left: 0, right: 0, height: 2, background: L.ink }} />}
          <StVault size={14} color={isFootage ? L.ink : L.ash} />
          <span className="hidden md:inline" style={{ fontSize: 13, color: isFootage ? L.ink : L.ash }}>Footage</span>
        </Link>
        ...
        <button onClick={() => setMenuOpen(o => !o)} aria-expanded={menuOpen} aria-label="Account"
          ...
          {isAccount && <span aria-hidden style={{ position: "absolute", top: -1, left: 0, right: 0, height: 2, background: L.ink }} />}
          <span style={{ width: 22, height: 22, borderRadius: 3, background: L.benchRaised, border: `1px solid ${L.rule}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, color: L.ash, overflow: "hidden" }}>
            {session?.user?.image
              // eslint-disable-next-line @next/next/no-img-element
              ? <img src={session.user.image} alt="" referrerPolicy="no-referrer" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
              : (session?.user?.name ?? "?").charAt(0).toUpperCase()}
          </span>
          <span className="hidden md:inline" style={{ fontSize: 13, color: isAccount || menuOpen ? L.ink : L.ash }}>Account</span>
        </button>
      </div>

      {menuOpen && (
        <div role="menu" style={{ position: "absolute", right: 8, bottom: 54, width: 240, background: L.bench, border: `1px solid ${L.rule}`, borderRadius: 3, overflow: "hidden", fontFamily: grotesque }}>
